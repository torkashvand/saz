...
# ---------------- legacy helpers (still part of the class) ----------------


def test_executor_agent_substitute_variables_round_trips_simple_template(
    agent: ExecutorAgent,
) -> None:
    """``_substitute_variables`` is the legacy ``{{var}}`` helper retained on
    the class. It is not used by ``ground()`` (which delegates to
    ``resolve_template``) but is part of the class API."""
    out = agent._substitute_variables(
        {
            "single": "{{user.email}}",
            "mixed": "Hi {{user.name}}, your id is {{id}}",
            "nested_list": ["{{user.name}}", "static"],
        },
        {"user": {"email": "a@b.com", "name": "alice"}, "id": 7},
    )
    assert out["single"] == "a@b.com"
    assert out["mixed"] == "Hi alice, your id is 7"
    assert out["nested_list"] == ["alice", "static"]


def test_executor_agent_substitute_variables_passes_through_primitives(
    agent: ExecutorAgent,
) -> None:
    # The legacy helper accepts any value (dict/list/str/primitive) — the
    # production annotation is narrower than the implementation. Cast via
    # Any to keep the test honest about the actual contract.
    sub: Any = agent._substitute_variables
    assert sub(7, {"x": 1}) == 7
    assert sub(True, {"x": 1}) is True
    assert sub(None, {"x": 1}) is None


def test_executor_agent_substitute_variables_raises_on_missing_variable(
    agent: ExecutorAgent,
) -> None:
    sub: Any = agent._substitute_variables
    with pytest.raises(ValueError, match="Variable path 'user.email' not found"):
        sub("{{user.email}}", {"user": {"name": "alice"}})


def test_executor_agent_get_nested_value_returns_leaf(agent: ExecutorAgent) -> None:
    assert agent._get_nested_value({"a": {"b": {"c": 9}}}, "a.b.c") == 9


def test_executor_agent_get_nested_value_raises_on_unknown_path(
    agent: ExecutorAgent,
) -> None:
    with pytest.raises(ValueError, match="not found in data"):
        agent._get_nested_value({"a": 1}, "a.b")
